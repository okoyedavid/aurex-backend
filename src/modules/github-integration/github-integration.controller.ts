...
export const createGitHubIntegrationController = ({ connectionService, externalAccessService }: { connectionService: GitHubConnectionService; externalAccessService: ExternalAccessReconciliationService }) => ({
  ...
  createInstallUrl: asyncHandler(async (req: Request, res: Response) => { const { businessId } = schemas.connectionSchema.shape.params.parse(req.validatedParams); return ok(res, await connectionService.createInstallUrl(businessId), "GitHub installation URL created"); }),
  completeInstallation: asyncHandler(async (req: Request, res: Response) => { const { businessId } = schemas.completeInstallationSchema.shape.params.parse(req.validatedParams); const { installationId, state } = schemas.completeInstallationSchema.shape.body.parse(req.validatedBody); return ok(res, await connectionService.completeInstallation(businessId, req.user!.id, installationId, state), "GitHub connection completed"); }),
