...
const githubConnectionSchema = new mongoose.Schema(
  {
    ...
    pendingStateHash: { type: String, default: null, select: false },
    pendingStateExpiresAt: { type: Date, default: null, select: false },
...
githubConnectionSchema.set("toJSON", { transform: (_doc, ret) => { const value = ret as { _id?: { toString(): string }; id?: string; pendingStateHash?: string; pendingStateExpiresAt?: Date }; if (value._id) value.id = value._id.toString(); delete value._id; delete value.pendingStateHash; delete value.pendingStateExpiresAt; return ret; } });
