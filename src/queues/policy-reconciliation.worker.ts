...
const processJob = async (job: Job<PolicyReconciliationJob>) => {
  ...
  if (job.data.type === "RECONCILE_WARP_DEMO") {
      ...
      const sandbox = await warpDemoSessionStore.getSession(demoRun.sessionId);
      if (sandbox.revision !== demoRun.revision || sandbox.activeRunId !== demoRun.runId) return { employeesProcessed: 0, staleDemoRun: true };
      ...
      const policyResult = await warpDemoSandboxEngine.reconcile({ sessionId: demoRun.sessionId, runId: demoRun.runId, revision: demoRun.revision, reason: job.data.reason, employeeChanged: demoRun.employeeChanged, asOfDate: new Date() });
      ...
      await warpDemoProgressService.appendEvent(demoRun.runId, { stage: "external_access", status: "success", title: "External access simulated", description: "No production external-access grant or privileged GitHub action was created." }, "external-complete");
      const auditChanged = demoRun.employeeChanged || assignmentsChanged;
      await warpDemoProgressService.appendEvent(demoRun.runId, { stage: "audit", status: "success", title: auditChanged ? "Audit evidence recorded" : "Audit state checked", description: auditChanged ? "Session-local employee and assignment changes were recorded." : "No employee or assignment change required a new audit record." }, "audit-complete");
      await warpDemoProgressService.appendEvent(demoRun.runId, { stage: "complete", status: "success", title: "Reconciliation complete", description: "The session's employee and policy assignment state is ready to refresh." }, "run-complete");
      await warpDemoProgressService.markCompleted(demoRun.runId, [...(demoRun.employeeChanged ? ["employee"] : []), ...(assignmentsChanged ? ["assignments"] : []), ...(auditChanged ? ["audit"] : [])]);
